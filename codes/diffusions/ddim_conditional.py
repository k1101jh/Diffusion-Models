...
class DDIM(AbstractDiffusion):
    ...
    def sample(self, model, n, labels, num_sampling_steps, cfg_scale=3, eta=0., gif_path=None, gif_term=1, clip_sample=False):
        model.eval()
        frames = []
        ...
        with torch.no_grad():
            ...
            for idx, t in tqdm(enumerate(sampling_steps), position=0):
                predicted_noise = model(x, (torch.ones(n) * t).long().to(self.device), labels)
                if cfg_scale > 0:
                    uncond_predicted_noise = model(x, labels, None)
                    predicted_noise = torch.learp(uncond_predicted_noise, predicted_noise, cfg_scale)
